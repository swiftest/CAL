...
class Attention(nn.Module):
    """
    caps_dim: 嵌入特征(胶囊特征)维数 8
    dim_head: 注意力的维度 16
    heads: 4 多头注意力机制里面的头数 4
    """
    def __init__(self, caps_dim, heads, dim_head, dropout):
        ...
        self.to_out = nn.Sequential(
            ...
            nn.Dropout(dropout)  # 注意力里面也有dropout
        )
    ...
    def forward(self, x):
        ...
        out = self.to_out(out)  # 恢复到原始嵌入特征的维度以进行残差学习  # (batch, 784, 8)
        return out
...
class ViT(nn.Module):
    ...
    depth: Transformer中编码器的层数(个数) 5
    heads: 多头注意力机制里面的头数 4
    mlp_dim: Transformer编码器中MLP的中间隐藏层的维数 8
    dim_head: 注意力的维度 16
    """
    def __init__(self, caps_num, caps_dim, depth, heads, mlp_dim, dim_head=16, dropout=0.1, emb_dropout=0.1, 
                 mode='CAF'):
        super().__init__()
        self.pos_embedding = nn.Parameter(torch.randn(1, caps_num, caps_dim))  # (1, 784, 8)注意，这里一定要是1！！！
    ...
    def forward(self, x):
        ...
        x = squash(x)  # 经过Transformer之后的注意力特征胶囊 (64, 784, 8)
        return x
...
class Decoder(nn.Module):
    def __init__(self, caps2_dim, band, patch_size=9):
        ...
        self.model = nn.Sequential(
            ...
            nn.ConvTranspose2d(256, self.band, 3, stride=2, padding=1)  # (batch_size, 176, 9, 9)
        )  
...
class CapsViT(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        out3 = rearrange(x, 'b (n h w) d -> b (n d) h w', h=self.spatial_size, w=self.spatial_size)  # (64, 128, 7, 7)
        x = self.vit(x)  # (batch, caps1_caps, caps1_dim) -> (64, 784, 8)
        out4 = rearrange(x, 'b (n h w) d -> b (n d) h w', h=self.spatial_size, w=self.spatial_size)  # (64, 128, 7, 7)
        ...
        if self.training:
            ...
            return x, x_recon, [out1, out2, out3, out4]
        else:
            ...
            return x, encode, [out1, out2, out3, out4]

        
class LossNet(nn.Module):
    def __init__(self, num_channels=[96, 48, 128, 128], inter_dim=128):
        super(LossNet, self).__init__()
        
        self.GAP = nn.AdaptiveAvgPool2d(output_size=1)
        
        self.FC1 = nn.Linear(num_channels[0], inter_dim)
        self.FC2 = nn.Linear(num_channels[1], inter_dim)
        self.FC3 = nn.Linear(num_channels[2], inter_dim)
        self.FC4 = nn.Linear(num_channels[3], inter_dim)
        
        self.linear = nn.Linear(4*inter_dim, 1)
...
class Margin_Recon_Loss(nn.Module):
    ...
    def forward(self, digital_caps_output, y, x, x_recon, m_plus=0.9, m_minus=0.1, lambda_=0.5):
        ...
        margin_loss = torch.sum(L, axis=1)  # (batch, )
        recon_loss = torch.mean(nn.MSELoss(reduction='none')(x_recon, x), axis=(1, 2, 3))  # (batch, )
