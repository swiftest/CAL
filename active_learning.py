...
def search_confident_samples(model_cap, model_ask, model_glom, candidate_loader):
    ...
    with torch.no_grad():
        for batch_idx, (batch_data, batch_target) in enumerate(candidate_loader):
            ...
            values, pred_cap = batch_pred_cap.topk(2, axis=1)  # (batch, 2)
            ...
            candidate_pred_cap = np.append(candidate_pred_cap, pred_cap[:, 0].cpu().numpy())
    ...
    num_candidate_sampls = len(candidate_bvsb)
    candidate_labels = torch.from_numpy(tar)
    num_classes = np.max(tar) + 1
    ind_bvsb = np.argsort(candidate_bvsb)
    ...
    for j in range(num_classes):
        ...
        print("{:.10f} \t Pred Label:{:2d} \t True Label:{:2d}".format(candidate_bvsb[target_index], j, tar[target_index]))
    print("**************************************************")
